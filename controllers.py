from models import Contact, ReceiptLaserLink, WarehouseReceipt, db
...
def get_consignees(req):
    # Execute the query and retrieve the results
    contacts_with_consignee_receipts = Contact.query.filter(Contact.warehouse_receipts_as_consignee.any()).all()  
    return get_formatted_receipts(contacts_with_consignee_receipts)
...
def get_warehouse_receipts_by_date(created_date, consignee_name):
    # Assuming you have a WarehouseReceipt model with a 'created_date' column
    warehouse_receipts = WarehouseReceipt.query.join(Contact,
        and_(
            WarehouseReceipt.consignee_id == Contact.id,
            Contact.contact_name == consignee_name
        )
    ).filter(
        WarehouseReceipt.is_archived == False,
        WarehouseReceipt.created_date == created_date
    ).all()
    return get_formatted_receipts(warehouse_receipts)
