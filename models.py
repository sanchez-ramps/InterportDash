...
if( os.environ.get('ENVIRONMENT') == 'development'):
    odoo_url = os.environ.get('DEVELOPMENT_ODOO_URL')
db = SQLAlchemy()
...
class PurchaseOrder(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    po_number = db.Column(db.String(1024))
...
class WarehouseReceipt(db.Model):
    ...
    magic_id = db.Column(db.String(50))
    ...
    def magic_link(self):
        magic_link = ""
        if self.magic_id:
            magic_link = f"https://transtrack.magaya.net/TransTracking?ID=14999&WH={self.magic_id}&XM=0&LG=en"
        return magic_link
